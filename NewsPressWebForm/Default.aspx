    ...
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
     <script type="text/javascript">
             ...
             $.getJSON("api/categories",
                 function (data) {
                     ...
                     $.each(data, function (key, val) {
                         ...
                         var row = '<td name="Name" onclick=tdClick(event,' + key + ')' + ' id=' + key + '>' + val.Name + '</td><td name="Path" id=' + key + '>' + val.ImagePath + '</td><td><button id="editBtn" onclick=editClick(event,' + key + ')>Edit</button><button id="deleteBtn" onclick=deleteClick(event,' + key + ')>Delete</button></td>';
         ...
         function tdClick(event, id) {
             id = id + 1;
             window.location.replace("/CategoryItems?catId="+id);
